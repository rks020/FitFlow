...
import 'manage_daily_tip_screen.dart';
  ...
  Future<void> _loadSubscriptionInfo() async {
    ...
    try {
      ...
      final orgResponse = await Supabase.instance.client
          .from('organizations')
          .select(
              'subscription_tier, subscription_type, trial_end_date, subscription_end_date')
      ...
      if (mounted) {
        setState(() {
          ...
          _subscriptionTier = (orgResponse['subscription_tier'] ?? 'free')
              .toString()
              .toUpperCase();
          ...
          if (orgResponse['subscription_end_date'] != null) {
            _subscriptionEndDate =
                DateTime.parse(orgResponse['subscription_end_date']);
          }
        });
        debugPrint(
            'Subscription loaded: Members=$_memberCount, Trainers=$_trainerCount, Tier=$_subscriptionTier');
      }
    } catch (e) {
    ...
    if (displayName.isNotEmpty && displayName != 'Kullanıcı') {
      ...
      if (names.length >= 2) {
        ...
        initials = names.first
            .substring(0, names.first.length > 1 ? 2 : 1)
            .toUpperCase();
      }
    }
      ...
      appBar: _profile?.role == 'owner'
          ? AppBar(
              ...
              title: Text(
                'Profil',
                style:
                    AppTextStyles.title1.copyWith(fontWeight: FontWeight.bold),
              ),
              centerTitle: true,
              actions: [
                Container(
                                ...
                                child: _profile?.avatarUrl == null
                                    ? Center(
                                        child: Text(
                                          initials,
                                          style:
                                              AppTextStyles.largeTitle.copyWith(
                                            color: AppColors.primaryYellow,
                        ...
                        if (_profile != null) ...[
                          ...
                          GlassCard(
                            ...
                            child: Column(
                              ...
                              children: [
                                _buildInfoRow(Icons.cake_outlined, 'Yaş',
                                    '${_profile?.age ?? "-"}'),
                                const Padding(
                                ...
                                _buildInfoRow(Icons.sports_tennis_rounded,
                                    'Hobiler', _profile?.hobbies ?? "-"),
                              ],
                            ),
                          ),
                        ],
                        ...
                        if (_profile?.role == 'trainer' ||
                            ...
                            child: _ProfileOption(
                              ...
                              onTap: () {
                                Navigator.push(
                                  ...
                                  MaterialPageRoute(
                                    builder: (context) =>
                                        const LowSessionNotificationsScreen(),
                                  ),
                                );
                              },
                            ),
                          ...
                          GlassCard(
                            child: _ProfileOption(
                              ...
                              title: 'Günün Sözünü Düzenle',
                              onTap: () {
                                Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                    builder: (context) =>
                                        const ManageDailyTipScreen(),
                                  ),
                                );
                              },
                            ),
  ...
  Future<void> _showDeleteOrganizationDialog() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        backgroundColor: const Color(0xFF222222),
        title: const Text('Salonu ve Hesabımı Sil',
            style: TextStyle(color: AppColors.accentRed)),
        content: const Text(
  ...
  Future<void> _executeDeleteOrganization() async {
    ...
    try {
      await Supabase.instance.client.functions.invoke('delete-user', body: {
        ...
        'user_id':
            _profile?.id ?? Supabase.instance.client.auth.currentUser?.id,
      });

      if (mounted) {
        CustomSnackBar.showSuccess(
            context, 'Salon ve hesap başarıyla silindi.');
      }
      ...
      child: Column(
        children: [
          ...
          Text(
            label,
            style:
                AppTextStyles.caption2.copyWith(color: AppColors.textSecondary),
          ),
          const SizedBox(height: 4),
          RichText(
            text: TextSpan(
