...
Deno.serve(async (req) => {
    ...
    try {
        const { email, password, first_name, last_name, specialty, organization_id } = await req.json();

        console.log('Request:', { email, organization_id });
        ...
        return new Response(
            JSON.stringify({ user: userData.user, message: 'Trainer created' }),
